...
int ESP32PWM::allocatenext(double freq) {
	...
	ESP_LOGE(TAG, "ERROR All PWM timers allocated! Can't accomodate %.3f Hz", freq);
	return -1;
}
...
bool ESP32PWM::checkFrequencyForSideEffects(double freq) {
	...
	allocatenext(freq);
	// Safety guard: MCPWM allocations (ESP32-S3) don't use LEDC timer indices.
	// Also protect against invalid timer index to avoid infinite loops / OOB access.
	const int timer = getTimer();
	if (isMCPWM || timer < 0 || timer > 3) {
		return true;
	}

	int safeCount = timerCount[timer];
	if (safeCount < 0) {
		safeCount = 0;
	} else if (safeCount > NUM_PWM) {
		safeCount = NUM_PWM;
	}

	for (int i = 0; i < safeCount; i++) {
		int pwm = timerAndIndexToChannel(timer, i);
		if (pwm < 0 || pwm >= NUM_PWM) {
			continue;
		}
		...
		if (pwm == pwmChannel)
			continue;
		if (ChannelUsed[pwm] != NULL)
			if (ChannelUsed[pwm]->getTimer() == timer) {
